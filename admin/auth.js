// ==========================================
// AUTH.JS - V3.0 Full-Page Auth System
// ✅ V3.0: Full-page auth (not popup), delayed page JS init
// Based on V2.2: Real-time role revocation + infinite loop fix + periodic fallback
// ==========================================
...
function injectAuthOverlay(pageName) {
    ...
    // ✅ V3.0: Hide page content immediately (full-page auth)
    const appContent = document.getElementById('appContent');
    if (appContent) {
        appContent.classList.remove('auth-visible');
    }
...
function hideAuthOverlay() {
    ...
    // ✅ V3.0: Show page content and remove body lock
    const appContent = document.getElementById('appContent');
    if (appContent) {
        appContent.classList.add('auth-visible');
    }
...
function forceAuthRevoke(reason) {
    ...
    // 2. ✅ V3.0: Hide page content immediately
    const appContent = document.getElementById('appContent');
    if (appContent) {
        appContent.classList.remove('auth-visible');
    }
    ...
    // 3. Show auth overlay with denied state
    ...
    // 4. Update denied text with reason
    setTimeout(() => {
...
function forceRoleRevoke(requiredRoles, userRoles) {
    ...
    // 3. Re-inject overlay if removed from DOM
    let overlay = document.getElementById('authOverlayNew');
    if (!overlay && currentPageName) {
        injectAuthOverlay(currentPageName);
        overlay = document.getElementById('authOverlayNew');
    ...
    // 4. Show overlay with No Access state
    if (overlay) {
        overlay.classList.remove('hidden');
        showNoAccessState(currentUserData || { roles: userRoles || [] });
    }

    // 5. ✅ DO NOT detach listener!
...
function initPageAuth(pageName) {
    ...
    // ✅ V3.0: Lock body and hide content immediately
    document.body.classList.add('auth-loading');
    const appContent = document.getElementById('appContent');
    if (appContent) {
        appContent.classList.remove('auth-visible');
...
console.log('Auth Module V3.0 Loaded (Full-Page Auth + Real-time Revocation + Loop Fix + Fallback Check)');